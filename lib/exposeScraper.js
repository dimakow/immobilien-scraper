...
exports.fnScrapeFromBodyAndReturnObject = function(body){
    ...
    if ($('.status-message,.status-warning,.margin-top-l').text().match("Angebot wurde deaktiviert") === null &&
        ...
        if ($(sSizeUsableSelector).text().match(sSizeUsableRegex) !== null) {
            nSizeUsable = $(sSizeUsableSelector).text().match(sSizeUsableRegex)[0].replace(".","").replace(',', '.')
            oReturn.sizeUsable = parseFloat(nSizeUsable);
        }
        if (nRentWithoutCharges !== null) {
            oReturn.rentWithoutCharges = parseFloat(nRentWithoutCharges[0].replace(".","").replace(",","."));
        }
        if (nRentCharges !== null) {
            oReturn.rentCharges = parseFloat(nRentCharges[0].replace(".","").replace(",","."));
        }
        if (nRentTotal !== null) {
            oReturn.rentTotal = parseFloat(nRentTotal[0].replace(".","").replace(",","."));
        }
